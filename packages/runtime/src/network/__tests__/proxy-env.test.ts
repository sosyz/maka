...
describe('getEnvWithProxy', () => {
  test('injects proxy env without overwriting user exports', () => {
    ...
    assert.strictEqual(out.HTTP_PROXY, 'http://existing:1234');
    assert.strictEqual(out.HTTPS_PROXY, 'http://127.0.0.1:7890');
    assert.strictEqual(out.NO_PROXY, 'localhost,127.0.0.1,::1,*.local');
  });
  ...
  test('emits socks5 and IPv6 URLs', () => {
    ...
    assert.strictEqual(out.HTTP_PROXY, 'socks5://[::1]:1080');
  });
});
